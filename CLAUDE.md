...
- 🔴 **R5 管线告警盲区（2026-06-13 实地发现）**：跨境 TLS 干扰期 R5 对 Upstash / Vercel Blob / **ntfy.sh 全部 SNI-RST**（`SSL_ERROR_SYSCALL`，只境内 newapi 通）。后果：①管线死在第一个 Upstash 调用 → 当晚连载停更（trial 第一晚不受影响，那条在 Vercel 跑）②**管线自身告警走不出去**（ntfy 同被阻断）→ 静默死在 journald（`journalctl -u fable-radio.service`、`systemctl show -p ExecMainStatus`）。retry timer 08-18 对**瞬态**块自愈（幂等）；持续块需 owner 介入或一个 **Vercel 侧 heartbeat（查 `pipeline:<date>.ranAt` 新鲜度，Vercel→ntfy 通）** 才能告警——后者尚未建
